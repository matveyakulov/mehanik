...
public interface ServiceAnnouncementRepository extends JpaRepository<ServiceAnnouncementEntity, Long> {
    ...
    @Query("SELECT new ru.neirodev.mehanik.dto.ServiceAnnouncementShowDTO(sa.id, sa.companyName, sa.address, sa.rating) " +
            "FROM ServiceAnnouncementEntity sa WHERE sa.service = :serviceType")
    List<ServiceAnnouncementShowDTO> findAllByService(ServiceType serviceType, Pageable pageable);

    @Query("SELECT new ru.neirodev.mehanik.dto.ServiceAnnouncementShowDTO(sa.id, sa.companyName, sa.address, sa.rating) " +
            "FROM ServiceAnnouncementEntity sa WHERE sa.service = :serviceType")
    List<ServiceAnnouncementShowDTO> findAllByService(ServiceType serviceType);
}
